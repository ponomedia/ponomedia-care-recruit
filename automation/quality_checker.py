...
NG_DOMAINS = [
    ...
    # 行政・公的機関（.go.jp は国の機関サイト — 小規模介護施設の公式サイトではない）
    "wam.go.jp", "mhlw.go.jp", "pref.", ".lg.jp", ".go.jp",
    # 住宅・不動産系行政サービス
    "mlit.go.jp", "satsuki-jutaku",
    # 介護エージェント・コンサル系（施設公式サイトでない）
    "caremane-agent.jp", "cho-sa.jp", "gengo-mikata.jp",
]
...
class QualityChecker:
    ...
    def _internal_check(
        ...
        # [3.3] AmeBloブログ記事ページを除外
        # ameblo.jp/USER/entry-XXXXXXXX.html は個別記事URL → 公式サイトではない
        if "ameblo.jp" in host and "/entry-" in parsed.path:
            return {"verdict": "fail", "reason": "AmeBloブログ記事URL（公式サイトではない）"}

        # [3.5] URLパス構造でポータル施設ページを検出
        # ポータルは「/施設カテゴリ/エリア/数字ID/」のような深いパス構造を持つ
        path = parsed.path
        path_segments = [s for s in path.split("/") if s]
        for seg in path_segments:
            if re.fullmatch(r'\d{4,}', seg):
                return {"verdict": "fail", "reason": f"ポータル施設詳細ページの疑い（数字ID: {seg}）"}
        if len(path_segments) >= 7:
            return {"verdict": "uncertain", "reason": f"URLパスが深すぎる（{len(path_segments)}階層）— ポータルの可能性"}
